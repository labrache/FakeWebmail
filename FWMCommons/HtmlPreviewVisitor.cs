...
namespace FWMCommons
{
    ...
    public class HtmlPreviewVisitor : MimeVisitor
    {
        ...
        string body;
        private readonly Boolean _protection;
        public Boolean hasTriggeredProtection { get; internal set; }
        /// <summary>
        /// Creates a new HtmlPreviewVisitor.
        /// </summary>
        /// <param name="mimeMessage">A Mime message</param>
        public HtmlPreviewVisitor(Boolean protection)
        {
            _protection = protection;
            hasTriggeredProtection = false;
        }
        ...
        void HtmlTagCallback(HtmlTagContext ctx, HtmlWriter htmlWriter)
        {
            ...
            else if(ctx.TagId == HtmlTagId.Image && _protection)
            {
                ctx.WriteTag(htmlWriter, false);
                ...
                htmlWriter.WriteAttribute("src", "./gris.png");
                hasTriggeredProtection = true;
            }
            else if (ctx.TagId == HtmlTagId.A && _protection)
            {
                ctx.WriteTag(htmlWriter, false);
                ...
                foreach (var attribute in ctx.Attributes)
                {
                    ...
                    if (attribute.Name.Equals("onclick", StringComparison.OrdinalIgnoreCase))
                        continue;
                    if (attribute.Name.Equals("target", StringComparison.OrdinalIgnoreCase))
                        continue;
                ...
                String b64href = Convert.ToBase64String(Encoding.UTF8.GetBytes(href));
                htmlWriter.WriteAttribute("onclick", String.Format("showLink('{0}')", b64href));
                hasTriggeredProtection = true;
            }
            else if (ctx.TagId == HtmlTagId.Style && _protection)
            {
                hasTriggeredProtection = true;
            }
            else
